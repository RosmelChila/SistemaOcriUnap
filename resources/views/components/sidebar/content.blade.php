<x-perfect-scrollbar as="nav" aria-label="main" class="flex flex-col flex-1 gap-4 px-3">
    ...
    <x-sidebar.link  title="Dashboard" href="{{ route('dashboard') }}" :isActive="request()->routeIs('dashboard')">
        <x-slot name="icon">
            <x-icons.dashboard class="flex-shrink-0 w-6 h-6 text-zinc-800 dark:text-white" aria-hidden="true" />
        </x-slot>
    </x-sidebar.link>
    ...
    <x-sidebar.link title="Usuarios" href="{{ route('usuarios') }}" :isActive="request()->routeIs('usuarios')">
        <x-slot name="icon">
            <x-icons.usuarios class="flex-shrink-0 w-6 h-6 text-zinc-800 dark:text-white" aria-hidden="true" />
        </x-slot>
    </x-sidebar.link>

    <x-sidebar.dropdown title="Filtrar" :active="Str::startsWith(request()->route()->uri(), 'buttons')">
        <x-slot name="icon">
            <x-icons.mundo class="flex-shrink-0 w-6 h-6 text-zinc-800 dark:text-white" aria-hidden="true" />
        </x-slot>
    ...
    <x-sidebar.dropdown title="Convenios" :active="Str::startsWith(request()->route()->uri(), 'buttons')">
        <x-slot name="icon">
            <x-icons.conve class="flex-shrink-0 w-6 h-6 text-zinc-800 dark:text-white" aria-hidden="true" />
        </x-slot>
    ...
    <x-sidebar.link title="Soporte" href="{{ route('soporte') }}" :isActive="request()->routeIs('soporte')">
        <x-slot name="icon">
            <x-icons.help class="flex-shrink-0 w-6 h-6 text-zinc-800 dark:text-white" aria-hidden="true" />
        </x-slot>
    </x-sidebar.link>
